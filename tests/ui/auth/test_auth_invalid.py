...
class TestAuthInvalid:
    ...
    @pytest.mark.asyncio
    async def test_wrong_email_shows_message(self, page: Page, user_data, expected_text):
    ...
    @pytest.mark.asyncio
    async def test_wrong_password_shows_message(self, page: Page, user_data, expected_text):
        auth_page = AuthPage(page)
        await auth_page.login(email=user_data["email"], password=user_data["password"])
        await expect(auth_page.login_error_locator()).to_have_text(expected_text)
    ...
    @pytest.mark.asyncio
    async def test_wrong_email_and_password_shows_message(self, page: Page, user_data, expected_text):
        auth_page = AuthPage(page)
        await auth_page.login(email=user_data["email"], password=user_data["password"])
        await expect(auth_page.login_error_locator()).to_have_text(expected_text)
    ...
    @pytest.mark.asyncio
    async def test_empty_email_message(self, page: Page, user_data, expected_text):
        auth_page = AuthPage(page)
        await auth_page.login(email=user_data["email"], password=user_data["password"])
    ...
    @pytest.mark.asyncio
    async def test_empty_password_shows_message(self, page: Page, user_data, expected_text):
        auth_page = AuthPage(page)
        await auth_page.login(email=user_data["email"], password=user_data["password"])
    ...
    @pytest.mark.asyncio
    async def test_empty_email_and_password_message(self, page: Page, user_data, expected_text_1, expected_text_2):
        auth_page = AuthPage(page)
        await auth_page.login(email=user_data["email"], password=user_data["password"])
    ...
    @pytest.mark.asyncio
    async def test_caps_password_email_shows_message(self, page: Page, user_data, expected_text):
        auth_page = AuthPage(page)
        await auth_page.login(email=user_data["email"], password=user_data["password"])
        await expect(auth_page.login_error_locator()).to_have_text(expected_text)
    ...
    @pytest.mark.asyncio
    async def test_caps_and_small_password_shows_message(self, page: Page, user_data, expected_text):
        auth_page = AuthPage(page)
        await auth_page.login(email=user_data["email"], password=user_data["password"])
        await expect(auth_page.login_error_locator()).to_have_text(expected_text)
